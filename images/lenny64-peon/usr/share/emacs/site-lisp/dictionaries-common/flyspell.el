...
;;
;; We define a flyspell-version function to let people know about this
...
(defun flyspell-post-command-hook ()
  "A wrapper to the `post-command-hook' used by flyspell to check a word in-the-fly.
Will run it only if flyspell-mode is enabled"
  (interactive)
  (when flyspell-mode (flyspell-post-command-hook-orig)))

; AMD: Done this way just to keep patch small, above check should be included in function

(defun flyspell-post-command-hook-orig ()
