...
(defun multi-prompt (separator
  ...
  "Completing read of a list of strings.
The first argument SEPARATOR should be the string (of length 1) to separate
the elements in the list. The second argument UNIQUE should be non-nil, if
each element must be unique. The remaining elements PROMPT, TABLE,
MP-PREDICATE, REQUIRE-MATCH, INITIAL, and HISTORY are the arguments to
`completing-read'."
...
(defun multi-prompt-next-must-match ()
  "Throw an error if the next input is invalid."
  (interactive)
